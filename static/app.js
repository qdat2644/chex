...
const I18N = {
  en: {
    ...
    reportClear: "No pathological findings exceeded calibrated thresholds. Frontal lung fields appear clear of acute radiopaque consolidation, pulmonary edema, or overt pleural effusion.",
    reportPrefix: "Automated analysis indicates",
    reportSuffix: "Clinical correlation and specialist radiologist overread are recommended.",
    reportHigh: "High suspicion of",
    reportMod: "moderate suspicion of",
    reportAnd: "and",
    // Print layout
    printSubHeader: "Chest Radiograph Automated Diagnostic & Pathology Report",
    printLblOrig: "Original Frontal Radiograph",
    printLblCam: "Grad-CAM Heatmap Overlay",
    printSec1: "1. Multi-Label Pathology Probabilities",
    printSec2: "2. Automated Radiological Impression",
    printThFinding: "Finding / Pathology",
    printThProb: "Probability",
    printThThresh: "Calibrated Threshold",
    printThStatus: "Status",
    printThSusp: "Clinical Suspicion",
    printFooterDisc: "Disclaimer: This automated report is generated by a deep neural network research prototype. It is not an FDA-cleared diagnostic device. Overread by a licensed radiologist is mandatory.",
    printSigLine: "Reviewing Radiologist Signature: ___________________________",
    diseaseNames: {
  ...
  vi: {
    ...
    // Report text templates
    reportClear: "Không có tổn thương bệnh lý nào vượt ngưỡng chẩn đoán. Phế trường hai bên sáng đều, không thấy hình ảnh đông đặc cấp tính, phù phổi hay tràn dịch màng phổi rõ rệt.",
    reportPrefix: "Kết quả phân tích tự động ghi nhận",
    reportSuffix: "Khuyến nghị đối chiếu lâm sàng và có ý kiến hội chẩn của bác sĩ chuyên khoa chẩn đoán hình ảnh.",
    reportHigh: "nghi ngờ cao về",
    reportMod: "nghi ngờ vừa về",
    reportAnd: "và",
    // Print layout
    printSubHeader: "Phiếu Báo Cáo Kết Quả Xét Nghiệm & Chẩn Đoán X-quang Tự Động",
    printLblOrig: "Ảnh X-quang Ngực Thẳng Gốc",
    printLblCam: "Bản Đồ Nhiệt Grad-CAM",
    printSec1: "1. Xác suất Đa Bệnh Lý & Ngưỡng Chẩn Đoán",
    printSec2: "2. Tóm Tắt Kết Luận Hình Ảnh Tự Động",
    printThFinding: "Tổn thương / Bệnh lý",
    printThProb: "Xác suất",
    printThThresh: "Ngưỡng tối ưu",
    printThStatus: "Trạng thái",
    printThSusp: "Mức độ nghi ngờ",
    printFooterDisc: "Miễn trừ trách nhiệm: Phiên bản nghiên cứu thử nghiệm bằng mạng nơ-ron học sâu. Không thay thế chẩn đoán xác định của bác sĩ chuyên khoa.",
    printSigLine: "Chữ ký Bác sĩ Chẩn đoán hình ảnh: ___________________________",
    diseaseNames: {
...
function applyLanguage(lang) {
  ...
  setText("print-sub-header", dict.printSubHeader);
  setText("print-lbl-orig", dict.printLblOrig);
  setText("print-lbl-cam", dict.printLblCam);
  setText("print-lbl-sec1", dict.printSec1);
  setText("print-lbl-sec2", dict.printSec2);
  setText("print-th-finding", dict.printThFinding);
  setText("print-th-prob", dict.printThProb);
  setText("print-th-thresh", dict.printThThresh);
  setText("print-th-status", dict.printThStatus);
  setText("print-th-susp", dict.printThSusp);
  setText("print-footer-disc", dict.printFooterDisc);
  setText("print-sig-line", dict.printSigLine);
  ...
  // Re-render findings & report box if data already present
...
// -----------------------------------------
// Build Localized Clinical Report
// -----------------------------------------
function buildLocalizedReport(data, lang) {
  if (!data || !data.findings) return "";
  const findings = data.findings;
  const dict = I18N[lang] || I18N.en;

  const positiveFindings = findings.filter((f) => f.positive);
  const highSuspicion = findings
    .filter((f) => (f.suspicion_level || "").toLowerCase().includes("high"))
    .map((f) => dict.diseaseNames[f.label] || f.label);
  const moderateSuspicion = findings
    .filter((f) => (f.suspicion_level || "").toLowerCase().includes("moderate"))
    .map((f) => dict.diseaseNames[f.label] || f.label);

  let reportText = "";
  if (!positiveFindings.length) {
    reportText = dict.reportClear;
  } else {
    const parts = [];
    if (highSuspicion.length) parts.push(`${dict.reportHigh} ${highSuspicion.join(", ")}`);
    if (moderateSuspicion.length) parts.push(`${dict.reportMod} ${moderateSuspicion.join(", ")}`);
    const findingsSummary = parts.join(` ${dict.reportAnd} `);
    reportText = `${dict.reportPrefix} ${findingsSummary}. ${dict.reportSuffix}`;
  }

  if (data.quality?.warnings?.length > 0) {
    reportText += ` (Note: ${data.quality.warnings.join("; ")})`;
  }
  return reportText;
}
...
function renderResponse(data) {
  ...
  const reportText = buildLocalizedReport(data, currentLang);
  if (reportText) showAlert(reportEl, reportText, "alert-info");
...
exportPdfBtn.addEventListener("click", () => {
  ...
  // Localized Impression for print
  document.getElementById("print-impression").textContent = buildLocalizedReport(data, currentLang);
